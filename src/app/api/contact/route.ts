...
export async function POST(request: NextRequest) {
  try {
    ...
    if (result.insertedId) {
      ...
      // but as a fallback.
      console.error(
        "API: Failed to insert message into MongoDB (no insertedId returned)."
      );
      return NextResponse.json(
        {
          error:
            "Failed to store message in database due to an unexpected issue.",
        },
        { status: 500 }
      );
    }
    ...
    if (
      ...
      errorMessage =
        "Database connection error. Please verify your MONGODB_URI in .env.local, check network access rules in MongoDB Atlas (if applicable), and ensure your database server is running.";
    } else if (
      error.message &&
    ...
    if (!process.env.MONGODB_URI || !process.env.MONGODB_DB_NAME) {
      console.error(
        "API Route Critical Error: MONGODB_URI or MONGODB_DB_NAME environment variable is not set in .env.local."
      );
      errorMessage =
        "Server configuration error: Database connection details (MONGODB_URI or MONGODB_DB_NAME) are missing in .env.local. Please contact the administrator or check your server setup.";
    }
